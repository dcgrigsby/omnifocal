...
- **Concrete test protocols in validate scripts (required):** The `implement` node prompt MUST specify the exact test protocol for each validate script — not just "run IT-1 through IT-5" but the actual commands (e.g., `curl -X POST -d '...' http://127.0.0.1:PORT/endpoint`), expected HTTP status codes, and response content checks. Abstract test descriptions like "start server, verify it works" produce hollow scripts that run `go test` or similar no-ops and exit 0. The prompt must say what tool to use (curl, not "test"), what to send, and what a valid response looks like.
- **Self-verification blocks in validate scripts (required):** Every validate script specified in the `implement` prompt MUST end with a self-verification block that asserts its own evidence was produced and is non-empty. The block must check: (1) manifest.json exists, (2) all expected IT-* IDs are present in the manifest, (3) at least one evidence file per scenario is non-empty and contains expected content (not just "ok" or "pass"). A script that exits 0 without producing real evidence is a false positive that wastes entire retry cycles.
- **Adversarial fidelity reviewer prompts (required):** Every `verify_fidelity` prompt MUST include the instruction "DO NOT TRUST tool node results — verify independently." The fidelity reviewer must read actual evidence artifact content (not just check file existence) and verify it contains real data from the target system. This catches hollow tests that passed verification gates by producing empty or placeholder evidence files.
